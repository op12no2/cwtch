...
Needs ```cwtch.c```, ```weights.h``` and ```makefile``` to make. Currently compiles OK under Linux and MSYS32 UCRT32 on Windows.  See latest commit for correct bench available via ```./cwtch bench quit```.
